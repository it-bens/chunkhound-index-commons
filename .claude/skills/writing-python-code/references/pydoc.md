...
`pydoc` reads the installed package's `__doc__` strings and signatures. For dependencies the project pins (`duckdb`, `duckdb_extension_vss`), `uv run python -m pydoc duckdb.DuckDBPyConnection` works because `uv` activates the project's virtualenv automatically.
