---
name: writing-tests
description: Use when authoring, editing, or reviewing a pytest test in chunkhound-index-commons — any change to a file under `tests/`.
---

# Writing Tests

The project's tests live under `tests/`, separate from `src/`. Shared pytest fixtures live in `tests/conftest.py`. Tests use plain `assert` and `pytest.raises`; tables are written via `@pytest.mark.parametrize`; temp filesystem state uses the built-in `tmp_path` fixture. Commons does not vendor large fixture artifacts — the one real-world fixture (`shopware_cli_index`) is located on disk in the sibling `chunkhound-index-compactor` repo and resolved via `pytest.skip` fallback.
...
Articulate the behavior in one sentence using business language. The test name is a paraphrase of that sentence (`test_reject_views`, not `test_duckdb_views_call`). If the sentence contains `and`, you have two tests. If it names an implementation choice (which dependency was called, internal call order, algorithmic decomposition), you have no test.
...
- `chunkhound_dir` / `fk_chain_db` / `hnsw_db` / `cosine_hnsw_db` / `shopware_cli_index` — shared fixtures defined in `tests/conftest.py`
...
Load `references/data.md` for the acceptable-vs-flagged paths table (absolute paths, ad-hoc cross-package borrows, dynamic globs, and the documented sibling-repo lookup pattern for `shopware_cli_index`), the descriptive-name conventions for paths and identifiers, the 5-line / 3-occurrence rule for helper extraction with `pytest.fixture` placement, and the large-fixture pattern for the externally-located `shopware-cli-chunks.duckdb`.
...
**Guard-clause isolation.** When the test targets one early-return in a function with multiple sequential guards (the canonical commons example is `schema.reject_unsupported_objects`, which runs five sequential refusal checks in order: non-`main` schema, view, UDT, generated column, self-ref FK), the arrange satisfies every other guard so the tested guard is the only possible exit. Open the public function under test, enumerate its sequential guards, and verify the arrange satisfies all guards above the targeted one. Otherwise the test passes through the wrong guard and proves nothing. The worked example lives in `references/behavior.md` §Guard clause isolation.
