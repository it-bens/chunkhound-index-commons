...
The project uses pytest's `tmp_path` for per-test filesystem state. No `pytest-xdist` today. No module-level mutable state in test files. `conftest.py` carries the shared fixtures; nothing else is global. The `shopware_cli_index` artifact in the sibling repo is read-only — every test that opens it does so read-only or copies first into `tmp_path`. `time.time()` / `datetime.now()` appear only inside fixture constructors whose values are never asserted; UUIDs and similar tokens, if introduced, must be fixed strings.
...
- Values produced fresh by a function-scoped fixture every test (`tmp_path`, the existing `chunkhound_dir` / `fk_chain_db` / `hnsw_db` / `cosine_hnsw_db`)
...
The existing `test_bundled_vss_path_missing_file_attribute_raises` and `test_bundled_vss_path_missing_binary_raises` show the `monkeypatch.setattr` pattern against `duckdb_extension_vss.__file__` — the patch is local to the test, restored automatically, and verifies the `RuntimeError` paths of `commons.vss.bundled_vss_path`.
...
| `random.random()`, `random.randint()` (without seed) | any test use that feeds an assertion |
...
The `hnsw_db` and `cosine_hnsw_db` fixtures use `random()` inside the DuckDB query to populate vectors. The values are never asserted, only counted; the existing tests assert `count(*)`, presence of indexes, and the metric returned by `capture_hnsw_metrics`, not vector contents. That is the boundary the rule sits at — non-determinism is fine when it does not feed an assertion.
...
| `session` | Read-only setup shared across the entire run. `shopware_cli_index` would be a candidate if profiling showed function-scope reopening dominated; today the fixture body is trivial (path resolution + skip), so function scope is fine. |
